...
function showResults() {
    ...
    currentQuestions.forEach((question, index) => {
        ...
        const userAnswer = getUserAnswerText(index);
        const correctAnswer = getCorrectAnswerText(index);
        
        detailsHtml += `
            <div style="margin: 15px 0; padding: 15px; border-radius: 8px; ${isCorrect ? 'background: #d4edda; border-left: 4px solid #28a745;' : 'background: #f8d7da; border-left: 4px solid #dc3545;'}">
...
function getUserAnswerText(questionIndex) {
    const question = currentQuestions[questionIndex];
    const userAnswer = userAnswers[questionIndex];
...
document.addEventListener('keydown', function(event) {
    ...
    if (question.type === 'single' || question.type === 'multi') {
        ...
        if (num >= 1 && num <= question.options.length) {
            if (question.type === 'single') {
                selectSingleOption(num - 1);
            } else {
                selectMultiOption(num - 1);
            }
        }
    }
